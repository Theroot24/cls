...
- **registry에는 대화 내용 일부가 평문으로 저장됩니다.** 세션마다 away summary 또는 마지막 사용자 프롬프트의 **첫 80자**가 `~/.claude/session-registry.md`에 그대로 기록됩니다(프로젝트 절대경로·세션 ID와 함께). registry는 소유자만 읽도록 `600` 권한으로 유지되며, 설치 이전부터 있던 파일도 **다음 세션이 기록되는 시점에 `600`으로 조정됩니다**(내용은 그대로 보존). 그때까지 기다리지 않고 바로 조이려면 `chmod 600 ~/.claude/session-registry.md`를 실행하세요. 공유 서버처럼 다른 계정이 같은 머신을 쓰는 환경이라면 설치 전에 이 점을 확인하세요.
